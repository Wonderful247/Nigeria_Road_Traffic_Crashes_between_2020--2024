...
def train_and_save_model(
    ...
    best_model = min(validation_scores, key=lambda name: validation_scores[name]["RMSE"])
    #  min() looks through all the model names and picks whichever one has the smallest RMSE value

    winner_model = candidate_models[best_model]   # grab the actual trained model object that won
    ...
    if show_progress:   # only print all this if the caller wants to see progress messages
        ...
        print(f"\nBest (lowest validation RMSE): {best_model}")
